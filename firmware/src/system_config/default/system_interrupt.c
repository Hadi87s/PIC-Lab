...
void __ISR(_CHANGE_NOTICE_VECTOR, ipl1AUTO) _IntHandlerChangeNotification(void)
{
    ...
    if (appData.isFlashing && PORTGbits.RG6 == 1)
    {
        appData.count = 0;
        appData.isFlashing = 0;
    }
    else if (!appData.isFlashing && PORTGbits.RG7 == 1)
    {
        appData.isFlashing = 1;
        PORTG = 0x0000;
    }
...
{
    if(appData.isFlashing)
    {
        appData.state = APP_STATE_FLASH;
    }
    else
    {
